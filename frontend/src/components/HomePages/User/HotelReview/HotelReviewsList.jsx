...
import { getHotelReviews, getHotelReviewSummary } from '../../../../services/HotelServices/reviewService';
...
const HotelReviews = ({ hotelId, hotelName }) => {
    ...
    const fetchSummary = useCallback(async () => {
        try {
            const response = await getHotelReviewSummary(hotelId);
    ...
    const fetchReviews = useCallback(async (page) => {
        try {
            setLoading(true);
            const response = await getHotelReviews(hotelId, page, pageSize);
    ...
    if (!summary && !loading) {
        return <div className="text-center py-12 text-gray-600 text-base">No reviews available</div>;
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-6">
            ...
            <div className="mb-8 pb-4 border-b border-gray-200">
                <h2 className="text-2xl font-semibold text-gray-900">Guest reviews for {hotelName}</h2>
            </div>
            ...
            {summary && (
                <div className="mb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="flex flex-col gap-4">
                            {renderCategoryBar('Facilities', summary.facilitiesAvg)}
                            {renderCategoryBar('Value for money', summary.valueForMoneyAvg)}
                        </div>
                        <div className="flex flex-col gap-4">
                            {renderCategoryBar('Cleanliness', summary.cleanlinessAvg)}
                            {renderCategoryBar('Location', summary.locationAvg)}
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="mt-12">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pb-4 border-b border-gray-200 gap-2">
                    ...
                    <span className="text-sm text-gray-600">
                        Showing {reviews.length > 0 ? currentPage * pageSize + 1 : 0} - {Math.min((currentPage + 1) * pageSize, totalElements)} of {totalElements} reviews
                    </span>
                </div>

                {loading ? (
                    <div className="text-center py-12 text-gray-600 text-base">Loading reviews...</div>
                ) : (
                    <>
                        {/* Reviews List */}
                                ...
                                >
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="font-semibold text-base text-gray-900">{review.userName}</span>
                                            </div>
                                        </div>
                                        <div className="bg-blue-900 text-white px-3 py-1.5 rounded-md font-bold text-base min-w-[40px] text-center">
                                    ...
                                    <div className="flex flex-wrap gap-4 mb-4 pb-3 border-b border-gray-100">
                                        ...
                                        <div className="flex items-center gap-1.5 text-sm text-gray-600">
                                            ...
                                            <span>{review.nights} {review.nights === 1 ? 'night' : 'nights'}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5 text-sm text-gray-600">
                                            <span className="text-base">📅</span>
                                    ...
                                    {[...Array(totalPages)].map((_, index) => {
                                        if (
                                            ...
                                            return (
                                                <button
                                                    key={index}
                                                    className={`min-w-[36px] px-3 py-2 border rounded text-sm transition-colors ${index === currentPage
                                            ...
                                            return <span key={index} className="px-1 py-2 text-gray-600 text-sm">...</span>;
                                        }
                                        return null;
                                    })}
                                </div>
