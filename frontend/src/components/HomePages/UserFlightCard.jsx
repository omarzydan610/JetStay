...
export default function UserFlightCard({ flight }) {
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      className="group relative bg-gradient-to-br from-sky-500 via-cyan-400 to-blue-500 rounded-xl p-4 md:p-5 shadow-lg hover:shadow-2xl transition-all duration-300 cursor-default w-full"
    >
      ...
      <div className="relative z-10 flex flex-col gap-2">
        {/* Airline header on top (prominent) */}
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-white/80 font-medium">AIRLINE</div>
            <div className="text-lg md:text-xl font-bold text-white leading-tight">{flight.airline?.airlineName || "N/A"}</div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-white truncate">{flight.departureAirport?.airportName || flight.departureAirport || "-"}</div>
            <div className="text-xs text-white/80 truncate">{flight.departureAirport?.city || "-"}, {flight.departureAirport?.country || "-"}</div>
          </div>

          <div className="mx-3 text-2xl text-white/90 font-bold">→</div>

          <div className="flex-1 text-right min-w-0">
            <div className="text-sm font-semibold text-white truncate">{flight.arrivalAirport?.airportName || flight.arrivalAirport || "-"}</div>
            <div className="text-xs text-white/80 truncate">{flight.arrivalAirport?.city || "-"}, {flight.arrivalAirport?.country || "-"}</div>
          </div>
        </div>

        <div className="h-px bg-white/20 my-2" />

        {/* Times and additional fields */}
              ...
              <div className="flex flex-col gap-1">
                {((flight.tripsTypes || flight.tripTypes) || []).slice(0,3).map((t, i) => (
                  <div key={i} className="text-xs">
                    <span className="font-medium">{t.typeName || t.name}</span>: <span>${t.price?.toFixed ? t.price.toFixed(2) : t.price}</span>
                  </div>
                ))}
              </div>
            )}
