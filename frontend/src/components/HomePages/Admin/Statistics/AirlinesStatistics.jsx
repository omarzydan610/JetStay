...
export default function FlightMonitoringSection() {
  ...
  useEffect(() => {
    ...
    const loadLastDayData = async () => {
      ...
      setLoading(true);
      try {
        const data = await adminMonitoringService.getFlightMonitoring(
          lastDayStart,
          lastDayEnd,
          0
        );
        ...
        console.error("Error auto-loading flight data:", err);
  ...
  const ticketPaymentStatusData = monitoringData?.ticketsByPaymentStatus
    ? Object.entries(monitoringData.ticketsByPaymentStatus).map(
        ([name, value]) => ({ name, value })
      )
    : [];

  const flightStatusData = monitoringData?.flightsByStatus
    ? Object.entries(monitoringData.flightsByStatus).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
  ...
  const paymentStatusData = monitoringData?.paymentsByStatus
    ? Object.entries(monitoringData.paymentsByStatus).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
  ...
  const paidPercentage = monitoringData
    ? (
        ((monitoringData.ticketsByPaymentStatus?.paid || 0) /
          (monitoringData.totalTickets || 1)) *
        100
      ).toFixed(1)
    : 0;
  ...
  return (
    <div className="space-y-8">
          ...
          <GlassCard>
            <div className="text-gray-500">
              ...
              <p className="text-sm mt-2">
                Select a date range and click "Get Data" to view flight
                statistics
              </p>
            </div>
          </GlassCard>
      ...
      {!loading && monitoringData && (
        <motion.div variants={itemVariants}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => navigate("/admin/tickets-details")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <EnhancedStatCard
              ...
              subValue={`${paidPercentage}% paid • ${
                monitoringData.ticketsByAirline?.length || 0
              } airlines`}
              icon={
                <svg
                  className="w-5 h-5"
            ...
            <EnhancedStatCard
              ...
              subValue={`Avg: $${
                monitoringData.totalTickets
                  ? (
                      monitoringData.totalRevenue / monitoringData.totalTickets
                    ).toFixed(2)
                  : "0.00"
              } per ticket`}
              icon={
      ...
      {/* Tickets by Airline */}
      {!loading &&
        monitoringData?.ticketsByAirline &&
        monitoringData.ticketsByAirline.length > 0 && (
          <motion.div variants={itemVariants}>
            <GlassCard>
              <h3 className="text-xl font-bold text-gray-900 mb-4">
      ...
      {/* Daily Tickets Trend */}
      {!loading &&
        monitoringData?.dailyTickets &&
        monitoringData.dailyTickets.length > 0 && (
          <motion.div variants={itemVariants}>
            <GlassCard>
              <h3 className="text-xl font-bold text-gray-900 mb-4">
                Daily Ticket Sales Trends
              </h3>
              <ResponsiveContainer width="100%" height={300}>
      ...
      {/* Payment Methods */}
      {!loading &&
        monitoringData?.paymentsByMethod &&
        monitoringData.paymentsByMethod.length > 0 && (
          <motion.div variants={itemVariants}>
            <GlassCard>
              <h3 className="text-xl font-bold text-gray-900 mb-4">
                Payments by Method
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {monitoringData.paymentsByMethod.map((method, index) => {
                  const totalPayments = monitoringData.paymentsByMethod.reduce(
                    (sum, m) => sum + m.count,
                    0
                  );
                  const percentage = (
                    (method.count / totalPayments) *
                    100
                  ).toFixed(1);
                  return (
                    <div
